...
protected:
  ...
  bool dedup_flush(ObjectContextRef obc);
  void cache_hit_set_create();
  void cache_hit_set_persist();
  bool cache_choose_mode(OpRequestRef op = OpRequestRef());
...
private:
  ...
  unordered_map<string, size_t> chunk_info_store;
};
